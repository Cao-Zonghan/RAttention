...
class SpatialMHSA2D(nn.Module):
    ...
    def _build_rope_cache(self, height, width, device, dtype):
        ...
        row_pair_count = pair_count // 2
        col_pair_count = pair_count - row_pair_count
        row_dim = row_pair_count * 2
        col_dim = col_pair_count * 2
        ...
        row_inv_freq = self.rope_base ** (
            -torch.arange(row_pair_count, device=device, dtype=torch.float32) / max(row_pair_count, 1)
        )
        col_inv_freq = self.rope_base ** (
            -torch.arange(col_pair_count, device=device, dtype=torch.float32) / max(col_pair_count, 1)
        )

        row_angles = row_positions[:, None] * row_inv_freq[None, :]
        col_angles = col_positions[:, None] * col_inv_freq[None, :]
        row_angles = row_angles.repeat_interleave(2, dim=-1)[:, None, :].expand(height, width, row_dim)
        col_angles = col_angles.repeat_interleave(2, dim=-1)[None, :, :].expand(height, width, col_dim)
        angles = torch.cat([row_angles, col_angles], dim=-1)

        self.rope_cos = angles.cos().to(dtype=dtype)
        self.rope_sin = angles.sin().to(dtype=dtype)
    ...
    def _reshape_rope_to_axis_patches(self, batch_size, height, width):
        if self.attention_axis == "row":
            num_patches = width // self.patch_size
            cos = self.rope_cos.view(height, num_patches, self.patch_size, self.head_dim)
            sin = self.rope_sin.view(height, num_patches, self.patch_size, self.head_dim)
            cos = cos.unsqueeze(0).expand(batch_size, height, num_patches, self.patch_size, self.head_dim)
            sin = sin.unsqueeze(0).expand(batch_size, height, num_patches, self.patch_size, self.head_dim)
        else:
            num_patches = height // self.patch_size
            cos = self.rope_cos.view(num_patches, self.patch_size, width, self.head_dim)
            sin = self.rope_sin.view(num_patches, self.patch_size, width, self.head_dim)
            cos = cos.permute(2, 0, 1, 3).unsqueeze(0).expand(
                batch_size, width, num_patches, self.patch_size, self.head_dim
            )
            sin = sin.permute(2, 0, 1, 3).unsqueeze(0).expand(
                batch_size, width, num_patches, self.patch_size, self.head_dim
            )
