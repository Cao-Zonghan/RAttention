...
class WaveletSubbandMHSA2D(nn.Module):
    ...
    def _get_rotary_position_table(self, batch_size, height, width, device, dtype):
        ...
        cache_key = (self.patch_size, self.rotary_dim, device.type, device.index, dtype)
        ...
        freqs = token_offsets.unsqueeze(-1) * inv_freq
        cos = torch.repeat_interleave(freqs.cos(), 2, dim=-1).view(
            1, 1, self.patch_size, self.rotary_dim
        )
        sin = torch.repeat_interleave(freqs.sin(), 2, dim=-1).view(
            1, 1, self.patch_size, self.rotary_dim
        )
