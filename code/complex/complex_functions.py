...
"""
import torch
from torch.nn.functional import relu, max_pool2d, max_pool3d, avg_pool2d, avg_pool1d, dropout, dropout2d, dropout3d, interpolate, sigmoid, tanh, leaky_relu, prelu
...
def complex_prelu(input, weight):
    return prelu(input.real, weight).type(torch.complex64)+1j*prelu(input.imag, weight).type(torch.complex64)
